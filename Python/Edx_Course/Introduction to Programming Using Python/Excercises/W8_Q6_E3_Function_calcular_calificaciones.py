...
def calculate_grades(file_name):
    ...
    my_list=[]
    new_list=[]
    for students in sorted(data):
        name, quiz1, quiz2, quiz3, quiz4 = students.strip().split(',')
        promedio=float((int(quiz1)+int(quiz2)+int(quiz3)+int(quiz4))/4)
        my_list.append(name)
        my_list.append(promedio)
        my_dictionary[name] = (promedio)
